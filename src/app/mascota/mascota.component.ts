...
export class MascotaComponent implements OnInit {
  ...
  confirmarEliminar() {
    if (this.idParaEliminar) {
      this.service.deleteMascotas(this.idParaEliminar).subscribe(
        (res) => {
          this.obtener();
        ...
        (error) => {
          this.error = true;
          this.mensaje = 'No es permitido eliminar la mascota';
          this.cerrarModal();
        }
      );
    }
  ...
  ngOnInit(): void {
    ...
    this.myModal = new bootstrap.Modal(
      document.getElementById('crearModal') as any
    );
    this.deleteModal = new bootstrap.Modal(
      document.getElementById('eliminarModal') as any
    );
  }
